...
//THIS IS COMMENTED TO TEST CLUSTER MODULE
...
//FOR CLUSTER AND REDIS test
if (cluster.isMaster) {
  console.log(`Master ${process.pid} is running`);
  const httpServer = http.createServer();

  var server = require('http').createServer();
  var io = require("socket.io")(server, { path: "/api/sockets", cors: {
    origin: '*'
  }});
  ...
  //const ios = new ServerIO(httpServer);
  let socketConnection = require("./routes/api/sockets/socketConnection");
  ...
  //io.on("connection", (socket) => {
    //console.log("conn created " + process.pid);
  //});
  //server.listen(PORT, () => console.log(`Server started at ${PORT}`));
}
//
